...
this.fn_comm_ajaxError = function(jqXHR, textStatus, errorThrown) {
	...
	if(jqXHR.responseText=="sessionOut"){
		alert("세션이 종료되었습니다.\n다시 로그인 하시기 바랍다.");
...
function getFileExtIcon(name){
	...
	var ext = ".bmp .doc .gif .hwp .hwpx .jpg .mp4 .pdf .png .ppt .swf .txt .xls .zip .xlsx";
...
function setConvertText(contents){
	var cont ="";
	if(contents != null){
		cont = contents.replace(/<script>/gi,"#bvsSscrpt#");	//script start replace
		cont = cont.replace(/<\/script>/gi,"#bvsEscrpt#");	//script end replace
	    cont = cont.replace(/<iframe/gi,"#bvsSfr#");	//iframe start replace
		cont = cont.replace(/<\/iframe>/gi,"#bvsEfr#"); 	//ifame end replace
	    cont = cont.replace(/onclick/gi,"#bvsClick#");		//onclick replace
	}
	return cont;
}


function getConvertText(contents){
	var cont ="";
	if(contents != null){
		cont = contents.replace(/#bvsSscrpt#/gi,"<script>");//script start replace
		cont = cont.replace(/#bvsEscrpt#/gi,"<\/script>");	//script end replace
		cont = cont.replace(/#bvsSfr#/gi,"<iframe");//iframe start replace
		cont = cont.replace(/#bvsEfr#/gi,"<\/iframe>");	//ifame end replace
	    cont = cont.replace(/#bvsClick#/gi,"onclick");  //onclick replace
	}
...
//음수,실수,정수만 포함하는 정규식함수
function fn_negativeDecimalNumber(obj) {
	$(obj).keyup(function(){
		// .replace(/[^-?\d.-]+/g,"") : 숫자,소수점,하이픈 제외한 문자 제거
		// .replace(/^(-)|-/g,"$1") : 중복 하이픈 제거
		// .replace(/^([^.]*\.)|\.+/g,"$1") :중복 소수점제거
		$(this).val($(this).val().replace(/[^-?\d.-]+/g,"").replace(/^(-)|-/g,"$1").replace(/^([^.]*\.)|\.+/g,"$1"));
	});
}
