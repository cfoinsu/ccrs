        ...
        init: function()
        {
          ...
          endEditListener: function(el) {
            ...
            el.on('focusout', function(e) {
                  ...
                  removeBtn = $(this).parent().find('.item-remove');
                  removeBtn.slideToggle();
                  spn.text($(this).parents().first().children('input[name="category"]').val());
                  if(spn.text() === '') spn.text('no title');
                  $(e.target).parents('.dd3-content').children('.dd-edit-box').first().slideToggle(50, function() {
                    var rmvBtn = $(e.target).parents('.dd3-content').find('.item-remove').first(),
                    edtBox = $(e.target).parents('.dd-edit-box').first(),
                    title = edtBox.find('input[name="category"]').first().val(),
                    item = edtBox.parents(opt.itemNodeName).first();
                    item.data('title', title);
                    spn.add(rmvBtn).slideToggle(50);
                  });
           ...
           createNewListItem: function(id, title) {
              var id = typeof id !== 'undefined' ? id : "NEW",
              title = typeof title !== 'undefined' ? title : 'New Item',
              el = this.el,
              opt = this.options,
              ...
              blueprint.find('input[name="category"]').first().val(title);
              blueprint.find('input[name="category"]').first().attr("data-catepidx",id=="NEW"?"NEW":id.split("_")[0]);
              blueprint.find('input[name="category"]').first().attr("data-cateidx",id=="NEW"?"NEW":id.split("_")[1]);
           ...
           deserialize: function(data, override) {
              ...
              var processItem = function(i, pref) {
                if(i.children) {
                  ...
                  item = _this.createNewListItem(i.id, i.title);
                  pref.append(item);
                  item.append(cref);
                  ...
                  var item = _this.createNewListItem(i.id, i.title);
                  pref.append(item);
                }
              }
