...
this.fn_comm_ajaxError = function(jqXHR, textStatus, errorThrown) {
	...
	if((jqXHR.responseText).indexOf("[CMS:sessionOut]")>-1){
		alert("세션 유지시간이 종료되었습니다.");
...
$.fn.fn_comm_setList = function(data, dataTemplateId, noDataTemplateId,
	...
	if (rowCountId) {
		var cnt = 0;
		if (isPaging) {
			var cntVal = String(list[0]["TOTAL_COUNT"]);
...
$.fn.rowspanRemove = function(colIdx, stdColIdx) {
	stdColIdx = __fn_comm_nvl(stdColIdx, colIdx);
	return this.each(function() {
		var that = null;
		var stdThat = null;
					...
					$_td.each(function(col) {
						...
						if ($(stdThis).html() == $(stdThat).html()) { // this=td:colIdx
							...
							$(that).attr('rowspan', rowspan); // do your
																...
																// the colspan
																// cell here
							$(this).remove(); // do your action for
											// the old cell here
							$(stdThis).remove(); // do your action
												// for the old cell here
						} else {
							that = this;
							stdThat = stdThis;
...
function getFileExtIcon(name){
	...
	var ext = ".bmp .doc .gif .hwp .hwpx .jpg .mp4 .pdf .png .ppt .pptx .swf .txt .xls .zip .xlsx";
...
function maxLengthCheck(object){
	if($(object).data("constr") != null && $(object).data("constr") != undefined){
		...
		if(constr.indexOf('reg')>-1){
			flag += 'reg';
		}
		if(constr.indexOf('email')>-1){
			flag += 'email';
		}
		if(constr.indexOf('koreng')>-1){
			flag += 'koreng';
		}
		if(flag=='num'){
			...
			$(object).keyup(function(){
				if (!(event.keyCode >=37 && event.keyCode<=40)) {
					var inputVal = $(this).val().toLowerCase();
					$(this).val(inputVal.replace(/[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/gi,''));
				}
				$(this).val($(this).val().replace(/[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/gi,""));
		    });
		}else if(constr=='email'){
			$(object).keyup(function(){
				$(this).val($(this).val().replace(/[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/gi,""));
		    });
		}else if(constr=='koreng'){
			$(object).keyup(function(){
				if (!(event.keyCode >=37 && event.keyCode<=40)) {
					var inputVal = $(this).val().toLowerCase();
					$(this).val(inputVal.replace(/[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"^0-9]/gi,''));
				}
				$(this).val($(this).val().replace(/[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"^0-9]/gi,""));
			});
		}
	}
...
function admCmnUserPop() {
	$("body").css("overflow-y","hidden");
	$('<iframe id="iframePop" src="/cms/CmnCon/CmnUserPop.do" frameborder="0"  style="right: 400;border: 0px currentColor; border-image: none; top: '+$(document).scrollTop()+'px; width: 100%; height: 100%; position: absolute; z-index: 999999;"/>').appendTo($("body #container"));
}


var fn_comm_file_ajax = function(url, param) {
	    ...
	    request.onload = function(e) {
	         ...
	         if(disposition != null){
	        	 ...
	        	 alert("다운로드에 문제가 발생했습니다. 관리자에게 문의하세요.");
	         }
	     };
	  request.send(param);	
};
