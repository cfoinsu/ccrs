...
$(document).ready(function() {
    ...
    function toggleQuickMenu(state) {
        ...
        $quickMenuBox.toggleClass('is-open', isExpanded);
    }

    // 💡 1. 마우스 호버 이벤트 (PC에서만 작동하도록 제한)
    if (!isTouchDevice) {
    ...
    // 💡 2. 버튼 클릭 이벤트 (모바일/PC 공통)
    $quickMenuBtn.on('click', function(e) {
        // 호버 이벤트와 충돌 방지 및 버블링 방지
        e.preventDefault();
        e.stopPropagation();
    ...
    // 외부 영역 클릭 시 메뉴 닫기 (모바일 편의성)
    $(document).on('click touchstart', function(e) {
        if (!$quickMenuBox.is(e.target) && $quickMenuBox.has(e.target).length === 0) {
            if ($quickMenuBtn.attr('aria-expanded') === 'true') {
                toggleQuickMenu(false);
            }
        }
    });
